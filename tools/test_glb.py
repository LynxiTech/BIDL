...
def load_config(args, USE_LYNGOR):
    ...
    globals.set_value('ON_APU', USE_LYNGOR)
    globals.set_value('FIT', USE_LYNGOR and 'soma_params' not in cfg["model"]["backbone"] or cfg["model"]["backbone"]['soma_params'] != 'channel_share')
    globals.set_value('MULTINET', args.multinet == 1)
...
def apu_infer(model_backbone_type, model_path, data_loader, dataset, chip_id, t, classes_num, args, data_name):
    """
    ...
    mode = args.mode
    sys.path.append("./")
    from lynadapter.lyn_sdk_model_glb import ApuRun
    ...
    if json_data['apu_x']['batch_size'] != json_data['apu_x']['n_slice']:
        raise ValueError('Models with unequal numbers of batch and n_slice are not supported yet.')
    ...
    for i, data in enumerate(tqdm(data_loader)):
        ...
        if mode == 1:
            for c in range(len(chip_id)):
                aruns[c].run_glb(data_img[batch_size * c:batch_size * (c + 1), ...],i)
        else:
            for c in range(len(chip_id)):
                aruns[c].run(data_img[batch_size * c:batch_size * (c + 1), ...],i)
 
        pp_per_start = time.time()
 
    # Collect APU outputs
    results = np.array(aruns[c].syn_op()).reshape(-1,classes_num)
...
def cpu_infer(model, data_name, data_loader, args, CLASSES, t):
    ...
    topk = (1, 5) if 'imdb' in data_name or 'luna16' in data_name else (1,)
    ...
    eval_results = {
        f'accuracy_top-{k}': validation_accuracy[i] / (batch_idx + 1)
...
def main():
    ...
    if USE_LYNGOR is True:
        if not USE_LEGACY: 
            ...
            if "it-" in args.config or "imdb" in args.config:
                model_compile(model.eval(),model_file,in_size,args.v,args.b,input_type="float16")
            else:
                model_compile(model.eval(),model_file,in_size,args.v,args.b)
        elif COMPILE_ONLY:
        ...
        if not COMPILE_ONLY:
            ...
            apu_infer(model_backbone_type,model_file,data_loader, dataset, chip_id,t,classes_num,args,data_name)
