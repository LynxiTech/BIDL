...
class CANN():
    ...
    def update(self, data, trajactory_mode = False):
        ...
        for i in range(0, seq_len):
            ...
            ## TODO replace odeint
            # dy /dt = f(y,t,\theta); odeint(f, y,t,args = \theta)
            # cur_du = odeint(int_u, u.flatten(), t,
            #                 args=(self.w, cur_stimulus.t().numpy(), self.tau, self.Inh_inp, self.k, self.dt))

            #cur_du = odeint(int_u, torch.tensor(u.flatten()), t, options['w']=(self.w, cur_stimulus.t().numpy(), self.tau, self.Inh_inp, self.k, self.dt))
            #print('u:', u.shape)
            #print('tensor_u:', torch.tensor(u.flatten()).shape)

            #int_u_para = {"Iext":cur_stimulus}

            cur_du = odeint(int_u, torch.tensor(u.flatten()), torch.tensor(t)) #**int_u_para
            cur_du = cur_du.numpy()
        ...
        if trajactory_mode:
            out = u_record
        # .fx : x.to(torchfloat16);scaling [0.00
        return out
...
def int_u(u, t):
    num = 128
    #w = np.zeros((num, num))
    w = torch.zeros((num, num))
    tau = torch.tensor(0.02)
    I_inh = torch.tensor(-.1)
    dt = torch.tensor(0.05)
    k = torch.tensor(1)
    Iext = torch.tensor(cur_stimulus)
    ...
    r1 = torch.square(u)
    #r2 = 1.0 + k * np.sum(r1)
    r2 = torch.tensor(1.0) + k * torch.sum(r1)
    r = r1 / r2
    #Irec = np.dot(r, w)
    r_new = torch.unsqueeze(r, 0)
    Irec = torch.dot(r, w)
