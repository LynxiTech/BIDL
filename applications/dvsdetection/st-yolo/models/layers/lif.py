...
class Lif(nn.Module):
    def __init__(self,
                 ...
                 fit: bool  # whether use resetwithdecay or not
                 ):
    ...
    def forward(self, x: pt.Tensor, h_and_c_previous: Optional[Tuple[pt.Tensor, pt.Tensor]] = None) \
            -> Tuple[pt.Tensor, pt.Tensor]:
        if h_and_c_previous is None:
            # generate zero states
            hidden = pt.zeros_like(x)
            cell = pt.zeros_like(x)
            h_and_c_previous = (hidden, cell)
        h_tm1, v = h_and_c_previous
        ...
        oupt = pt.tanh(v)
        h_c_tuple = tuple([oupt, v])
        return oupt, h_c_tuple
...
class Lif2d(Lif):
    ...
    def __init__(self,
                 norm_state=False,
                 mode='spike', memb_mode=MEMB_MODE, soma_params=SOMA_PARAMS, noise=0, hidden_channel=None, on_apu=False,
                 fit=False
                 ):
        ...
        super(Lif2d, self).__init__(norm, mode, memb_mode, SOMA_PARAMS, noise, on_apu, fit)
