...
model = dict(
    ...
    backbone=dict(
        type='ResNetLifItout',
        timestep=8,h=224,w=224,input_channels=3,
        depth=18, nclass=27,
        down_t=[1, 'avg'],
        noise=1e-5,
...
train_pipeline = [
    dict(type='LoadFramesInFolder', n_frame=8, down_t=(1.0, 2.0, 0.1), dropout=0.05, random=True),
    ...
    dict(type='RandomResizedCropVideo', center=(0, 0), size=224, scale=(0.4, 1.0), ratio=(3 / 4, 4 / 3)),
    #dict(type='NormalizeVideo', **img_norm_cfg),
    dict(type='ToTensorTypeDict', keys=['img'], dtype='float32'),
    dict(type='ToTensor', keys=['gt_label']),
]
test_pipeline = [
    dict(type='LoadFramesInFolder', n_frame=8, down_t=(1.5,), random=False),
    dict(type='CropPadByRatioVideo', croppad=None, allowed_ratio=[1, 4 / 3], min_padd=.1, max_padd=.2,
        pad_mode='constant'),
    dict(type='ResizeVideo', size=(224, -1)),
    dict(type='CenterCropVideo', crop_size=224),
    #dict(type='NormalizeVideo', **img_norm_cfg),
    dict(type='ToTensorTypeDict', keys=['img'], dtype='float32'),
]
data = dict(
