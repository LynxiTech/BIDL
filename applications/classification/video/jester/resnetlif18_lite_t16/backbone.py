...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        #self.lif2 = Lif2d(mode=mode, noise=noise, soma_params=soma_params, hidden_channel=hidden_channels[1])
        self.relu = nn.ReLU()
...
class ResNetLifReluItout(nn.Module):
    arch_settings = {
    ...
    def __init__(self,
        ...
        super(ResNetLifReluItout, self).__init__()
        assert down_t[0] == 1
    ...
    def _make_layer(block, ci, co, blocks, stride, noise, mode='spike', soma_params='all_share', hidden_channels=None):
        ...
        block = BasicBlock_Relu if isinstance(layers[0], BasicBlock) else Bottleneck_Relu
    ...
    def forward(self, xis):
        if self.ON_APU:
            ...
            return output.squeeze(-1).squeeze(-1)#(1,10)
        else:
            x5s = []
