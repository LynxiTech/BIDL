...
class MultiLIF(Function):
    @staticmethod
    def forward(ctx, input, time, v_reset, tau, v_threshold):
        timesteps = time
        spike_list = []
        v = 0.
        for t in range(timesteps):
            x = input[t].unsqueeze(0)
            # Neuronal charge：
            v = v + (x - (v - v_reset)) / tau

            # Neuronal fire：
            spike_d = ((v - v_threshold) >= 0).float()

            # Neuronal reset：
            v = (1. - spike_d) * v + spike_d * v_reset

            spike_list.append(spike_d)

        spike = torch.cat(spike_list, 0)
...
mylif = MultiLIF.apply


class MultiStepLIFNode(nn.Module):
    def __init__(self, tau: float = 2., v_threshold: float = 1., v_reset: float = 0., detach_reset: bool = False):
    ...
    def forward(self, input):
        if globals.get_value('isCompile'):
            out = mylif(input, 4, self.v_reset, self.tau, self.v_threshold)
            return out
        else:
            ...
            for t in range(timesteps):
                ...
                # Neuronal fire：
                spike_d = ((self.v - self.v_threshold) >= 0).float()
...
class BNAndPadLayer(nn.Module):
    ...
    def forward(self, input):
        isModified = globals.get_value('isModified')
        ...
        if self.pad_pixels > 0: 
            ...
            if isModified:
                TB, _, H, W = output.shape
                pad_values_tmp1 = pad_values.repeat(TB, 1, 1, W)
                output = torch.cat([pad_values_tmp1, output], 2)
                output = torch.cat([output, pad_values_tmp1], 2)
                TB, _, H, W = output.shape
                pad_values_tmp2 = pad_values.repeat(TB, 1, H, 1)
                output = torch.cat([pad_values_tmp2, output], 3)
                output = torch.cat([output, pad_values_tmp2], 3)
            else:
...
class SepConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if isModified:
            TB, C, H, W = x.shape
            x = self.lif1(x)
            x = self.bn1(self.pwconv1(x))#.reshape(TB, -1, H, W)
            x = self.lif2(x)
            x = self.dwconv(x)
            x = self.bn2(self.pwconv2(x))#.reshape(TB, -1, H, W)
        else:
            T, B, C, H, W = x.shape
            x = self.lif1(x)
...
class MS_ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if isModified:
            ...
            x = self.Conv(x) + x
            x_feat = x
            x = self.bn1(self.conv1(self.lif1(x)))
            x = self.bn2(self.conv2(self.lif2(x)))
            x = x_feat + x
        else:
            T, B, C, H, W = x.shape
...
class MS_MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        if isModified:
            ...
            x = self.fc1_lif(x)
            x = self.fc1_conv(x)
            x = self.fc1_bn(x).reshape(TB, self.c_hidden, N).contiguous()

            x = self.fc2_lif(x)
            x = self.fc2_conv(x)
            x = self.fc2_bn(x).reshape(TB, C, H, W).contiguous()
        else:
            T, B, C, H, W = x.shape
            N = H * W
            x = x.flatten(3)
...
class MS_Attention_RepConv_qkv_id(nn.Module):
    ...
    def forward(self, x):
        ...
        if isModified:
            ...
            x = x.transpose(2, 3)
            x = self.attn_lif(x).reshape(TB, C, H, W)
            x = x
            x = self.proj_conv(x)
        else:
            T, B, C, H, W = x.shape
...
class MS_DownSampling(nn.Module):
    ...
    def forward(self, x):
        ...
        if isModified:
            ...
            _, _, H, W = x.shape
            x = self.encode_bn(x).reshape(TB, -1, H, W).contiguous()
        else:
            T, B, _, _, _ = x.shape
