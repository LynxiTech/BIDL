...
def binary_accuracy(preds, y, topk=1, thrs=0.):
    """
...
def binary_accuracy_numpy(preds, y, topk=1, thrs=0.):
    """
    Returns accuracy per batch, i.e. if you get 8/10 right, this returns 0.8, NOT 8
    """
    ...
    #round predictions to the closest integer
    res =[]
    rounded_preds = np.round(preds).squeeze()
...
def accuracy(pred, target, topk=1, thrs=0.):
    ...
    if isinstance(pred, torch.Tensor) and isinstance(target, torch.Tensor):
        ...
        if len(pred.shape)==1 or pred.shape[1]==1:
            res = binary_accuracy(pred, target)
        else:
            res = accuracy_torch(pred, target, topk, thrs)            
    elif isinstance(pred, np.ndarray) and isinstance(target, np.ndarray):
        if pred.shape[1]==1:
            res = binary_accuracy_numpy(pred, target,topk)
        else:
